...
public class LevelModel {
	...
	public LevelModel(short[][] map, short[] balls) {
		this.map = map;
		this.balls = balls;
	 ...
	 * @return
	 */
	private int addRandomBalls(int number, int score, List<Integer> updates) {
		if (isWin()) {
			return score;
		}
		int k = 0;
		for (int i = 0; i < map.length; i++) {
			for (int j = 0; j < map[i].length; j++) {
				if (isEmptyGround(map[i][j]) && !hasBall(map[i][j]) && !hasFuture(map[i][j])) {
					...
					score = putBall(i, j, ball, score, updates);
				}
			}
		}
		if (k <= number) {
			...
			for (int i = 0; i < map.length; i++) {
				for (int j = 0; j < map[i].length; j++) {
					if (isEmptyGround(map[i][j]) && !hasBall(map[i][j]) && !hasFuture(map[i][j])
							&& l < randomNumbers.length) {
						if (k++ == randomNumbers[l]) {
							map[i][j] = setFutureBall(map[i][j], balls[rand.nextInt(balls.length)]);
							l++;
							logger.logInfo("Set future ball to map[" + i + "][" + j + "].");
						}
					}
				}
			}
	...
	private int countBalls(int column, int raw, int directionColumn, int directionRaw) {
		...
		while (same) {
			...
			raw += directionRaw;
			if (column >= 0 && raw >= 0 && column < map.length && raw < map[column].length
					&& sameLineBalls(map[column][raw], type)) {
				result++;
			} else {
				same = false;
			}
	...
	private void removeBall(int column, int raw, List<Integer> updates) {
		...
		for (int i = 0; i < directions.length; i++) {
			...
			raw += directions[i][1];
			if (column >= 0 && column < map.length && raw >= 0 && raw < map[column].length
					&& sameLineBalls(map[column][raw], type)) {
				removeBall(column, raw, updates);
			}
			column -= directions[i][0];
			raw -= directions[i][1];
		}
	...
	private int checkAndCleanBall(int column, int raw, int score, List<Integer> updates) {
	...
	private int putBall(int column, int raw, short type, int score, List<Integer> updates) {
		if (isEmptyGround(map[column][raw]) && !hasBall(map[column][raw])) {
			...
			score = checkAndCleanBall(column, raw, score, updates);
			// model.notifyView(ILinesView.NOTIFY_UPDATE_LEVEL);
	...
	private static boolean checkPath(short[][] map, int fromCol, int fromRaw, int toCol, int toRaw) {
		...
		if (fromCol == toCol && fromRaw == toRaw) {
			...
			for (int i = 0; i < directions.length; i++) {
				...
				if (newCol < map.length && newCol >= 0 && newRaw < map[fromCol].length && newRaw >= 0
						&& !hasBall(map[newCol][newRaw])) {
	...
	public int moveBall(int fromCol, int fromRaw, int toCol, int toRaw, int score, List<Integer> updates) {
		if ((!hasBall(map[toCol][toRaw])) && checkPath(fromCol, fromRaw, toCol, toRaw)) {
			short oldType = map[fromCol][fromRaw];
			logger.logInfo("LevelModel.moveBall from " + fromCol + "," + fromRaw + " to " + toCol + "," + toRaw
					+ " type=" + oldType);
			score = putBall(fromCol, fromRaw, BALL_EMPTY, score, updates);
			score = putBall(toCol, toRaw, oldType, score, updates);
			// Otherwise: user wins the battle
		}
		return score;
	}
	
	public boolean isSelectable(int col, int raw) {
		return hasBall(map[col][raw]);
	}
	
	public boolean isEmpty(int col, int raw) {
		return isEmptyGround(map[col][raw]) && !hasBall(map[col][raw]);
	}

	public int addRandomBalls(int score, List<Integer> updates) {
		return addRandomBalls(newBallAddition, score, updates);
	}
	...
	public static final short setFutureBall(short value, short ball) {
		return (short) ((short) (value & ~FUTURE_BALL_MASK) | (ball >> 5));
	}

	public static final boolean sameLineBalls(short value1, short value2) {
		return hasBall(value1) && hasBall(value2) && ((value1 & BALL_MASK) == (value2 & BALL_MASK));
	}

	public static final boolean hasCell(short value) {
	...
	public static final int getFutureNumber(short value) {
		return ((value & FUTURE_BALL_MASK) >> 2) - 1;
	}

	public static final int getBallNumber(short value) {
		return ((value & BALL_MASK) >> 7) - 1;
	}
